...
function ParamPanel() {
          ...
          if (schemaHash !== lastSchemaHash) {
            ...
            if (modifyMode === 'json') {
              ...
              if (schema.input_schema?.properties) {
                Object.entries(schema.input_schema.properties).forEach(([key, prop]) => {
    ...
    try {
      ...
      if (currentSchema.input_schema?.properties) {
        Object.entries(currentSchema.input_schema.properties).forEach(([key, prop]) => {
          modifiedSchema.input_schema.properties[key] = {
            ...prop,
  ...
  const renderFormInput = (key: string, prop: PropertySchema) => {
    const commonProps = {
      style: { width: '100%' }
    };
